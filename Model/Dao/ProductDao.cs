...
namespace Model.Dao
{
    ...
    public class ProductDao
    {
        ...
        {
            string a = HttpUtility.HtmlEncode(keywword);
            totalRecord = db.Products.Where(x => x.Name.Contains(a)).Count();
            var model = db.Products.Where(x => x.Name.Contains(a)).OrderBy(x => x.CreatedDate).Skip((pageIndex - 1) * pageSize).Take(pageSize).ToList();
            return model;
        }
