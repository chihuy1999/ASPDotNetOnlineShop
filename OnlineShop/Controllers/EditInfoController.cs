...
namespace OnlineShop.Controllers
{
    public class EditInfoController : BaseController
    {
        ...
        public ActionResult Info(User user)
        {
            ...
            {
                var dao = new UserDao();
                if (dao.CheckPassword(user.Password) == 1)
                {
                    ModelState.AddModelError("", "password nhập vào phải <= 25 kí tự");
                }
                else if (dao.CheckPassword(user.Password) == 2)
                {
                    ModelState.AddModelError("", "password nhập vào phải >= 8 kí tự");
                }
                else
                {
