...
namespace OnlineShop.Controllers
{
    public class ForgotPasswordController : Controller
    {
        ...
        public ActionResult Index(ForgotPassModel model)
        {
            ...
            {
                var dao = new UserDao();
                if (dao.CheckPassword(model.Username) == 1)
                {
                    ModelState.AddModelError("", "Username nhập vào phải <= 25 kí tự");
                }
                else if (dao.CheckPassword(model.Username) == 2)
                {
                    ModelState.AddModelError("", "Username nhập vào phải >= 8 kí tự");
                }
                else
                {
